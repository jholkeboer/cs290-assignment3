...
var bar = function(doubleArray){
	var copyArr = doubleArray;
	for (var i = 0; i < copyArr.length; i++) {
		if (isNaN(copyArr[i])) {
			return false;
		}
		else {
			copyArr[i] = doubleArray[i] * 2.0;
		}
	}
	return true;
};
...
function parseGit(logArray) {
	...
	for (var i = 0; i < logArray.length; i++) {
		...
		var newMessage = '';
		newHash = logArray[i].substring(0,7);
		newDate = new Date(logArray[i].substring(8,39));
		newMessage = logArray[i].substring(40,56);
		newMessage = newMessage.replace('"','');
		newMessage = newMessage.replace('','');
		gitLogArray.push(new GitLog(newHash, newDate, newMessage));
	}
	return gitLogArray;
}
//end your code
